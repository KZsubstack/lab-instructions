...
**Your answer:** The Denny's dataset has 1,643 rows and 6 columns. Each row represents one Denny's location, more specifically "locations of 1643 Denny's Restaurants in the United States" according to the description of the dataset. The variables are it's address, city, state, zip, longitude, and latitude
...
**Your answer:** The La Quinta dataset has 909 rows and 6 columns. Each row represents locations of La Quinta Inn in the US. The variables are it's address, city, state, zip, longitude, and latitude.
...
**Your ideas:**
We could look for unusual state or province codes in the state column that belong to other countries instead of U.S. states, using the states dataset.

Since both datasets have long and lat columns we could know the min/max of both long and lat for the area of the USA and filter any entry that is outside of that range. 
...
**How many Denny's locations are outside the US?** 0

**If there are any, which state/province codes appear?** None
...
**Complete the code:**
```{r add-dennys-country,  eval = TRUE}
...
**Complete the code:**
```{r add-laquinta-country,  eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
laquinta <- laquinta %>%
 ...
 **Knit, commit with message "Completed Exercises 7-8", and push your changes.**
...
- State with most locations: California
- State with fewest locations: Delaware

**La Quinta:**

- State with most locations: Texas
- State with fewest locations: Veracruz Mexico

**Is this surprising? Why or why not?**

This is not very surprising because the states with the most locations for both Denny’s and La Quinta are California, Texas, and Florida. These are some of the largest states in the country, so it makes sense that both restaurant and hotel chains would have many locations there. It also makes sense that their rankings do not match exactly, since hotels and restaurants may expand differently depending on travel. This does not necessarily weaken the claim that there is a higher chance of finding a La Quinta near a Denny’s, or vice versa, because the overall number of locations in a state is different from how closely the two chains are clustered together within that state. The smaller counts in places like Delaware are also not surprising because those states have less land area and smaller populations.
---
...
- State with highest density: District of Columbia
- Density value: 29.265437518

**La Quinta:**

- State with highest density: Rhode Island
- Density value: 1.294590553
...
```{r plot-all-locations,  eval = TRUE}
...
Many Denny’s and La Quinta locations are near each other and appear in a few large clusters together, which gives Mitch Hedberg’s joke some support. But if we take the joke literally, there are still many La Quinta locations with no Denny’s nearby, so the two do not consistently overlap in Texas. From a broader perspective, though, the joke may hold somewhat true on average.


**Compare North Carolina and Texas: In which state does the joke seem more accurate?**

Definitely Texas, in North Carolina there are many isolated Denny's, this could be because in Texas many establishments will cluster around the larger cities, however overall Texas has more locations that overlap both establishments


 **Knit, commit with message "Completed Exercises 11-12", and push your changes.**

...
**What are the cases?** Each case represents a birth recorded in North Carolina in 2004, with information about the mother, father, pregnancy, and baby.
...
**Numerical variables:** fage, mage, weeks, visits, gained, weight
...
ggplot(data = ncbirths, aes(y = weight)) +
  geom_boxplot() +
  labs(title = "Distribution of baby weights")

# Boxplot for other numerical variables as needed
ggplot(data = ncbirths, aes(y = weeks)) +
  geom_boxplot() +
  labs(title = "Distribution of pregnancy length in weeks")

ggplot(data = ncbirths, aes(y = gained)) +
  geom_boxplot() +
  labs(title = "Distribution of weight gained by mother")
```

**Are there outliers?** Yes. The boxplots show low outliers in baby weight and pregnancy length, and both low and high outliers in weight gained by the mother.
...
**How does this compare to 7.43 pounds?** The observed mean is lower than the 1995 value of 7.43 pounds by about 0.18 pounds.
...
**Independence:** Births in the sample come from different mothers and one birth does not influence another. The observations are independent.
...
There are 714 observations. Yes, this sample size is far above the rule of thumb of 30, so it is large enough.


**Random sampling:** The data was drawn as a random sample from North Carolina births in 2004, so the sample represents the population well.


**Conclusion: Are conditions satisfied?** Yes, all three conditions hold, so we proceed with simulation-based inference.
...
In context: The data provides strong evidence that the average weight of babies born to white mothers in North Carolina in 2004 differs from the 1995 value of 7.43 pounds.
...
ggplot(data = ncbirths, aes(x = habit, y = weight)) +
  geom_boxplot() +
  labs(
    title = "Baby weight by mother smoking habit",
    x = "Smoking habit",
    y = "Birth weight (pounds)"
  )
```

**What does the plot show about the relationship?** Babies born to nonsmoking mothers tend to weigh more than babies born to smoking mothers. The median weight for the nonsmoker group sits higher than the median for the smoker group.



**Which group has higher median weight?** Nonsmokers have a higher median birth weight.
...
**How many observations remain after removing NAs?** 999
...
**Null hypothesis (H₀):** The average weight of babies born to smoking mothers equals the average weight of babies born to nonsmoking mothers. $H_0: \mu_{nonsmoker} - \mu_{smoker} = 0$



**Alternative hypothesis (Hₐ):** The average weight of babies born to smoking mothers differs from the average weight of babies born to nonsmoking mothers. $H_A: \mu_{nonsmoker} - \mu_{smoker} \ne 0$
...
**Independence:** Births in the sample are independent of each other and the two groups (smoker and nonsmoker) are independent.



**Sample size in each group:**
```{r check-habit-counts, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
ncbirths_habitgiven %>%
  count(habit)
```

Nonsmokers: 873 observations
Smokers: 126 observations

**Are these large enough?** Yes, both groups have well over 30 observations.


**Random sampling:** The data is a random sample of North Carolina births, so the sample represents the population.



**Conclusion:** All conditions are satisfied for simulation-based inference on the difference in means.
...
```{r hypothesis-test-habit, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
...
**The observed difference is:** 0.316 pounds

**The p-value is:** about 0.02

**Interpretation:** Since the p-value of 0.02 is less than 0.05, we reject the null hypothesis. The data provides evidence of a difference in mean birth weight between babies born to nonsmoking mothers and babies born to smoking mothers.
...
**The 95% confidence interval is:** (0.05, 0.58)

**Interpretation in context:**

We are 95% confident that the true difference in mean birth weight between babies of nonsmoking mothers and babies of smoking mothers in North Carolina is between 0.05 and 0.58 pounds, with nonsmokers having heavier babies on average.
...
**The age cutoff is:** 35 years

**Explanation of method:** I grouped the data by the mature variable and looked at the minimum and maximum age in each group. Younger mothers range up to age 34 and mature mothers start at age 35. The cutoff sits at 35 years old.
...
**H₀:** The proportion of low birth weight babies is the same for mature and younger mothers. $H_0: p_{mature} - p_{younger} = 0$

**Hₐ:** The proportion of low birth weight babies is higher for mature mothers than for younger mothers. $H_A: p_{mature} - p_{younger} > 0$

**Note:** This is a **one-sided test** because we're testing if mature mothers have a *higher* proportion.

**Verify conditions:**

**Independence:** Births in the sample are independent of each other, and the two groups of mothers are independent.

___________________________________________________________________________

**Sample size:**
```{r check-maturemom-counts, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
ncbirths %>%
...
**Are conditions satisfied?** Yes. Both groups have at least 10 low birth weight babies and 10 babies of normal weight, and the data was randomly sampled, so all conditions hold.



**Calculate observed proportions:**
```{r observed-proportions, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
ncbirths %>%
  filter(!is.na(mature), !is.na(lowbirthweight)) %>%
  group_by(mature) %>%
...
Younger mothers: 0.10
Mature mothers: 0.13

**Run the hypothesis test:**
```{r hypothesis-test-mature, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
...
**The p-value is:** about 0.17

**Conclusion:** Since the p-value of 0.17 is greater than 0.05, we fail to reject the null hypothesis. The data does not provide evidence that mature mothers have a higher proportion of low birth weight babies than younger mothers.



**If significant, construct a 95% confidence interval:**
```{r ci-mature, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
...
**The 95% confidence interval is:** Since the difference was not significant, the confidence interval is not required here. If reported, the bootstrap interval falls around (-0.03, 0.10), which contains 0 and matches our conclusion.

**Interpretation:** The interval contains 0, which agrees with the hypothesis test result that we fail to reject the null. We do not have evidence of a higher proportion of low birth weight babies among mature mothers.

...
```{r yaml-raw-to-rendered, fig.fullwidth=TRUE, echo = FALSE, fig.height=2, fig.margin=FALSE, fig.width=2, out.width="60%"}
...
```{r update-author-name-commit, fig.fullwidth=TRUE, echo = FALSE, fig.height=2, fig.margin=FALSE, fig.width=2, out.width="60%"}
...
Based on the help file there are 1,846 rows and 3 columns that the `datasaurus_dozen` file have.
...
dino_data %>%
  summarize(r = cor(x, y))
```
The output of the correlation coefficient in example 2 is -0.06447185.
...
Approximately $-0.0629611$. Compared to the dino dataset ($-0.0645$), the value is very similar and close to zero, even though the visual plots of the two datasets are very different.
...
```{r circle-data, eval = TRUE}
circle_data <- datasaurus_dozen %>%
  filter(dataset == "circle")
```

```{r circle-plot, eval=TRUE, fig.fullwidth=TRUE, fig.height=5, fig.width=5}
ggplot(data = circle_data, mapping = aes(x = x, y = y)) +
  geom_point()
```

```{r circle-correlation, eval = TRUE}
circle_data %>%
  summarize(r = cor(x, y))
```
...
The correlation coefficient for the circle dataset (-0.0683) is very similar to the dino dataset (-0.0645). Although it is slightly more negative, both values are close to zero.

- What do you notice about the correlation coefficients for dino, star, and circle?

The correlation coefficients for the dino, star, and circle datasets are all very similar and close to zero, despite the datasets having very different visual patterns. This shows that correlation by themselves doesn't necessarily fully capture things about the structure or shape of the data. It also shows why visualization is essential in data analysis
...
```{r all-viz, eval=TRUE, fig.fullwidth=TRUE, fig.height=7, fig.width=7}
...
```{r fig-resize-global, echo = FALSE, , fig.height=2, fig.margin=FALSE, fig.width=2, out.width="60%"}
...
```{r fig-resize-local, fig.margin = FALSE, echo = FALSE, fig.height=2, fig.width=2, out.width="60%"}
...
```{r theme-highlight, echo=FALSE, fig.height=2, fig.margin=FALSE, fig.width=2, out.width="60%"}

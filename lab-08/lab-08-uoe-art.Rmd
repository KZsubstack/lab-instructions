 ...
 **Remember to commit and push after completing each script!**
...
#remember to switch eval = TRUE to TRUE when you have your data
 ...
 **Knit, commit with message "Completed Exercise 9", and push your changes.**
...
- Number of pieces with missing artist: 106	
- Number of pieces with missing year: 507
...
```{r year-histogram, eval = TRUE, fig.pos = "H"}
# Remember to change eval=FALSE to eval=TRUE!
ggplot(data = uoe_art, aes(x = year)) +
  geom_histogram(binwidth = 20) +
  labs(
    title = "Distribution of Edinburgh College of Art Collection by Year",
    x = "Year",
...
The small column around 1700 looks like the outlier it's isolated from the rest of the collection. and also this warning

This warning: Warning	
Warning: [38;5;232mRemoved 781 rows containing non-finite outside the scale range (`stat_bin()`).[39m


 **Knit, commit with message "Completed Exercises 10-11", and push your changes.**
...
# Remember to change eval=FALSE to eval=TRUE!
uoe_art %>%
  filter(year < 1750)  # Look for unreasonable years
```

**Visit the link for this piece and find the correct year:** 
The dates listed are ranges (e.g., 1700-1799, 1700-1850), not single years.
**Why didn't our code capture it correctly?**
str_extract("\\d{4}") grabs only the first 4-digit number it finds, so for a range like "1700-1799" it extracts "1700" and ignores the rest. The dates are ranges, not single years, so there's no single correct year to extract.
...
uoe_art <- uoe_art %>%
  mutate(year = if_else(
    title == "Chair seat",          # Condition: if title equals the problematic one
    1775,                    # Then: set year to correct value
...
**Most common artist:** 
Unknown, Known would be Emma Gillies
**Number of pieces:** 
319 for unknown 144 for Emma Gilles
**Do you know this artist?** 
No I do not know Emma Gilles nor unknown



**Why might the university have so many of their pieces?**

Emma Gillies was could've been a student or faculty member at Edinburgh College of Art, so the university collected her work.


 **Knit, commit with message "Completed Exercises 12-13", and push your changes.**
...
**Number of pieces with "child" in the title:** 8

 **Knit, commit with message "Completed Exercise 14", and push your changes.**
